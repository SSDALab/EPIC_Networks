...
network.layout.animate.useAttribute <- function(nw, layout.par=list(x='x',y='y')){
  ...
  if (is.null(layout.par$x) | is.null(layout.par$y)){
    stop("useAttribute layout must have layout.par listed argument with elements named 'x' and 'y' giving the names of the vertex attributes containing the x and y coordinates to use")
  }
  xattr <- layout.par$x
  yattr <-layout.par$y
  # check that they correspond to a valid attribute and the attribute is numeric
  if (!xattr%in%list.vertex.attributes(nw) ){
    stop(paste("unable to find vertex attribute",xattr,"containing x coordinate information"))
  }
  if (!yattr%in%list.vertex.attributes(nw) ){
    stop(paste("unable to find vertex attribute",yattr,"containing y coordinate information"))
  }
