  ...
  # locate potentially missing activity attributes
    ...
    if (!return.tea){
      if(any(sapply(vals,length)>1)){
        warning("Multiple attribute values matched query spell for some edges, only earliest value used\n")
...
activate.vertex.attribute <- function (x, prefix, value, onset=NULL, terminus=NULL,length=NULL,at=NULL, v=1:network.size(x), dynamic.only=FALSE){
  ...
  if((min(v) < 1) || (max(v) > network.size(x))){  # combinatorial breakage:
...
activate.edge.value<-function(x, prefix, value, onset=NULL, terminus=NULL,length=NULL,at=NULL, 
                                   e=seq_len(network.edgecount(x)), dynamic.only=FALSE){
  activate.edge.attribute(x=x,prefix=prefix,value=value, onset=onset,terminus=terminus,length=length,at=at,e=e,dynamic.only=dynamic.only)
}
...
activate.edge.attribute <-function(x, prefix, value, onset=NULL, terminus=NULL,length=NULL,at=NULL, 
  ...
  if(!attrname%in%list.edge.attributes(x)){
    if(prefix%in%list.edge.attributes(x) & !dynamic.only){
      #delete the old attribute, it will effectively be replaced with the new one
...
get.vertex.attribute.active <- function(x, prefix, onset=NULL, terminus=NULL,length=NULL, at=NULL, rule = c("any", "all"), na.omit = FALSE, null.na = TRUE, active.default = TRUE, dynamic.only=FALSE, require.active=FALSE,return.tea=FALSE,unlist=TRUE){
  ...
  if (!attrname%in%list.vertex.attributes(x) & !dynamic.only){
    ...
    if (!return.tea){
      ...
      if(length(extravals)>0){
        warning("Multiple attribute values matched query spell for some vertices, only earliest value used\n")
        for (index in extravals){
          vals[[index]]<-vals[[index]][[1]]
        }
      }
  ...
  if (require.active){
    vals[which(!is.active(x,onset=onset,terminus=terminus,v=1:network.size(x),rule=rule,active.default=active.default))] <- NA
  }
  #  handle null.na
  if (null.na){
...
##########################################################################
deactivate.vertex.attribute <- function (x, prefix, onset=NULL, terminus=NULL,length=NULL,at=NULL, v=1:network.size(x), dynamic.only=FALSE){
  #possibly duplicate list for when setting multiple verticies
  #check that argument is a network
  if(!is.network(x)){
...
deactivate.edge.attribute <-function(x, prefix, onset=NULL, terminus=NULL,length=NULL,at=NULL,e=seq_len(network.edgecount(x)), dynamic.only=FALSE){
  #check that argument is a network
  if(!is.network(x)){
...
list.vertex.attributes.active <-function(x, onset=NULL, terminus=NULL,length=NULL, at=NULL, na.omit=FALSE,null.na=FALSE,rule = c("any", "all"), v=1:network.size(x), active.default = TRUE, dynamic.only=FALSE, require.active=FALSE){
  
  if(!is.network(x)){
    stop("list.vertex.attributes.active requires that the first argument be a network")
  }
  ...
  if((min(v) < 1) || (max(v) > network.size(x))){  # combinatorial breakage:
    ...
    currently.active <- sapply(gsub(".active","",attributes.active),function(attr){
      any(!is.na(get.vertex.attribute.active(x,attr,onset=onset, terminus=terminus,length=length, at=at, rule = rule, na.omit = na.omit, null.na = null.na, active.default = active.default, dynamic.only=dynamic.only, require.active=require.active,return.tea=TRUE)))
      })                                        
  if(dynamic.only){
...
##########################################################################
list.edge.values.active <-function(x, onset=NULL, terminus=NULL,length=NULL, at=NULL, na.omit = FALSE, null.na = TRUE, rule = c("any", "all"), e=seq_len(network.edgecount(x)), active.default = TRUE, dynamic.only=FALSE, require.active=FALSE){
  # validate inputs
  ...
  if(!is.null(at)) {
    if(!is.vector(at) || !is.numeric(at))
      stop("Singular time points given by the 'at' argument must be a numeric vector in list.edge.value.active\n")
    if(!(is.null(onset) && is.null(terminus) && is.null(length)))
      stop("Query intervals must be specified by exactly 1 of {at, onset+terminus, onset+length, length+terminus}")
  } else {
    if(!is.null(onset) && (!is.vector(onset) || !is.numeric(onset)))
      stop("Onset times must be a numeric vector in list.edge.value.active\n")
    if(!is.null(terminus) && (!is.vector(terminus) || !is.numeric(terminus)))
      stop("Terminus times must be a numeric vector in list.edge.value.active\n")
    if(!is.null(length) && (!is.vector(length) || !is.numeric(length) || any(length < 0)))
      stop("Interval lengths must be a non-negative numeric vector in list.edge.value.active\n")
    if(!is.null(onset)) {
      if(!xor(is.null(terminus),is.null(length)))
        stop("Query intervals must be specified by exactly 1 of {at, onset+terminus, onset+length, length+terminus}")
    } else {
...
list.network.attributes.active <-function(x, onset=NULL, terminus=NULL,length=NULL, at=NULL, na.omit = FALSE, null.na = TRUE, rule = c("any", "all"), active.default = TRUE, dynamic.only=FALSE){
  ...
  if(!is.null(at)) {
    ...
    currently.active <- sapply(gsub(".active","",active.attributes),function(attr){
      any(!is.na(get.network.attribute.active(x,attr,onset=onset, terminus=terminus,rule = rule, dynamic.only=dynamic.only,return.tea=TRUE)))
    })                                        
