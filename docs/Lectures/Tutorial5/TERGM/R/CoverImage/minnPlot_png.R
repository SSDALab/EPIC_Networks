...
gplot.spatial<-function(x,coord,bb,list.edges,list.vertex,background=NULL,list.background){
	#require(geosphere)
	plot.new()
	...
	if(!is.null(background)){
		do.call("plot",c(list(x=background),list.background,list(add=TRUE)))
	}
...
points<-sf.pt.proj[which(deg[dego[1000]]==deg)[1],]
index<-overlay(points,sf.proj)
index<-unique(index)
index<-index[!is.na(index)]
poly<-buildBB(sf.proj[index,],.0001)
...
coord<-coordinates(sf.pt.proj)
col<-rgb(1,0,0,.01)
col2<-rgb(1,0,0,.1)
#,colormodel = "cmyk" -- for pdf
...
gplot.spatial(sf.net,coord,bb=bbox(sf.proj),list.vertex=list(col=col,pch=19,cex=.0001),list.edges=list(col=rgb(0,0,0,.3),lwd=.3),background=sf.proj,list.background=list(border=rgb(0,0,1,.1)))
...
par(fig=c(.25, 4, .6, 3.1)/10,new=TRUE,mgp=c(0,0.2,0),tcl=-.2)#,bg="white")
gplot.spatial2(sf.net,coord,bg=sf.proj,bb=bbox(poly),list.vertex=list(col=col2,pch=19,cex=.3),list.edges=list(col=rgb(0,0,0,.3)))
dev.off()
